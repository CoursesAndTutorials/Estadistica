...
Transformemos esa **tabla** en un **data frame**, lo que facilita su tratamiento:

```{r, eval=TRUE}
#tabla anterior
NomTabla=table(mineria$Mineral)

#recuperando valores
Nombres=names(NomTabla)
conteo=as.vector(NomTabla)
porcentajes=as.vector(prop.table(conteo))

#conviertiendo a dataFrame
NomDf=data.frame(var=Nombres,abs=conteo,rel=porcentajes)
...
La misma tabla la usamos para graficar usando la biblioteca **ggplot2**:

```{r}
library(ggplot2)

base=ggplot(NomDf, aes(x=var,y=abs))
...
```{r, eval=TRUE}
# tabla simple anterior, pero ya "ordinalizada":
OrdTabla=table(accidentes$Tama)

#recuperando valores
Nombres=names(OrdTabla)
conteo=as.vector(OrdTabla)
porcentajes=as.vector(prop.table(conteo))

# calculando acumulaciones
cum_conteo=cumsum(conteo) # suma de conteo
cum_porcentajes=cumsum(porcentajes) # suma de conteo en %

#creando DF
OrdDf=data.frame(var=Nombres,
                    abs=conteo,rel=porcentajes,
                    abs_acu=cum_conteo,rel_acu=cum_porcentajes,stringsAsFactors = F)
#resultado
OrdDf
```
...
```{r}
base= ggplot(OrdDf, aes(x=var,y=abs)) + 
      scale_x_discrete(limits=Nombres) # indicando orden
...
```{r}
intervTabla=table(intervalos)

Nombres=names(intervTabla)
conteo=as.vector(intervTabla)
porcentajes=as.vector(prop.table(conteo))
...
NumDf=data.frame(var=Nombres,
                 abs=conteo,rel=porcentajes,
                 abs_acu=cum_conteo,rel_acu=cum_porcentajes,
                 stringsAsFactors = F)
...
base=ggplot(NumDf, aes(x=var,y=abs)) + scale_x_discrete(limits=Nombres)
...
histOrd= base + geom_histogram(bins=7) 
histOrd 
```

Al histograma le podemos añadir su polígono de frecuecias:

```{r}
histOrd1= histOrd + geom_freqpoly(bins=7,col='blue')
histOrd1
```

Y su ojiva de dataos acumulados:

```{r}
histOrd1  + stat_bin(aes(y=cumsum(..count..)),geom="line",bins=7,col='red') 
```
