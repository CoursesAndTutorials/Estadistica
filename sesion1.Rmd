...
El estudio de la estadística trae consigo el familiarizarse con una serie de conceptos:

* __POBLACION__. Todos los estudios estadísticos se hacen tratando de aproximarse al conocimiento de una población. En ese sentido, se asume que los miembros de una población tienen algo en común. Eso en común tiene variación (no es una constante), lo que amerita el estudio estadístico. Durante el estudio estadístico, se puede encontrar que la población no se definió bien, lo que perjudicó el proceso de muestreo. 

* __MUESTRA__. La muestra es parte de la población. Se habla de muestra representativa si esta es producto de un proceso que ha respetado principios de selección probabilísticos; de caso contrario, se le denomina muestra de coveniencia. Sólo la representativa permite generalización.

* __PARAMETRO y ESTADIGRAFO__. Los valores que describen una población se denominan parámetros. Siendo las poblaciones tan grandes, se suele usar la muestra para estimar los parámetros desde la muestra. Los valores que describen la muestra son los estadígrafos.

* __TIPO de dato__. Los datos son de diverso tipo. Los programas de análisis estadístico no siempre identifican de manera adecuada esos tipos. Si tomamos una hoja de calculo como referencia, una columna debe tener un sólo tipo de dato. En general los tipos se clasifican en:
...
No todas la columnas son variables de estudio. Algunas de ellas son los __identificadores__, valores que sirven para identificar cada fila o unidad de análisis.
...
Cuando los datos están '_limpios_', y se sabe su ubicación, R recupera el archivo rápidamente:
...
Toda la información ha sido '_transferida_' usando el simbolo **=** (también se puede utilizar **<-**) a **datosLeidos**. El comando **read.csv** se utilizó pues corresponde al tipo de archivo original en _GoogleDrive_. Un archivo de este tipo es similar a un archivo en Excel, de hecho puedes abrirlo en Excel fácilmente.
...
El comando _c()_  es muy utilizado pues sirve para concatenar valores en un **vector**. Justo hemos visto cómo sirve para crear dos vectores. 
...
Hice sólo un cambio, y R me entrega para esa variable el conteo de cada _modalidad_ o categoría.
